...
function setImageAvatarNews(){
    $('#avatarNews').html("");
    let selected = $('input[type=radio]:checked');
    $('#avatarNews').append(`<input name="nAvatar" type="text" value="${selected.val()}" class="d-none"><img width="100%" src="<%= process.env.DOMAIN_SERVER %>/uploads/${selected.val()}">`);
}

function changeCheckBoxToRadio(){
    $('#btnSetImage').attr('onclick', 'setImageAvatarNews()')
}

let myTypeImage = 'radio';
let myNameImage = 'imageAvatar';

function removeImageDetail(ele){
    ele.closest(".col-sm-4").remove();
}
